      ...
      try {
        ...
        if (shouldRespond) {
          const result = useMultiStep
            ? await runMultiStepCore({ runtime, message, state, callback: callback as HandlerCallback })
    ...
    try {
      for (const providerName of providers) {
        ...
        traceActionResult.push({
          ...
          error: success ? undefined : (providerResult as any)?.error,
        });
        if (callback) {
          await callback({
...
const postGeneratedHandler = async ({
  ...
  const RM = state.providerData?.find((pd: {providerName: string}) => pd.providerName === 'RECENT_MESSAGES');
  if (RM) {
